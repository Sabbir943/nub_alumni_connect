...
export const CATEGORIES = [
  ...
  { name: 'Career Advice', icon: <FiBriefcase size={14} />, color: 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400' },
  { name: 'Technology', icon: <FiCompass size={14} />, color: 'bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400' },
  { name: 'Events', icon: <FiBookOpen size={14} />, color: 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400' },
...
export default function BlogSidebar({ selectedCategory, onCategoryChange, currentUserEmail, isMobile }) {
  ...
  return (
    <aside className={`${isMobile ? 'block w-full' : 'hidden lg:block w-80 lg:w-96 xl:w-[420px] shrink-0'} space-y-4 ${!isMobile ? 'sticky top-20 self-start max-h-[calc(100vh-5rem)] overflow-y-auto scrollbar-thin pb-6' : ''}`}>
      {/* Categories */}
      <Section title="Categories" icon={<FiGrid size={14} />}>
      ...
      {/* Trending Posts */}
      <Section title="Trending Posts" icon={<FiTrendingUp size={14} />}>
        {loading ? (
            ...
            {trending.map((post, i) => (
              <div key={post._id} className="flex gap-3 group">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-medium text-zinc-700 dark:text-zinc-300 line-clamp-2 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors cursor-pointer">
      ...
      {/* Top Contributors */}
      <Section title="Top Contributors" icon={<FiUsers size={14} />}>
        {loading ? (
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="animate-pulse flex items-center gap-3">
                ...
                <div className="flex-1 space-y-1.5">
                  ...
                  <div className="h-2.5 bg-zinc-200 dark:bg-zinc-700 rounded-full w-1/3" />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-3">
            {contributors.map((user) => (
              <div key={user.email} className="flex items-center gap-3 group">
                <div className="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-[10px] font-bold shrink-0 overflow-hidden">
                  {user.avatar ? (
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium text-zinc-700 dark:text-zinc-300 truncate group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                    {user.name}
                  </p>
      ...
      {/* Recent Activity */}
      <Section title="Recent Activity" icon={<FiActivity size={14} />}>
        {loading ? (
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="animate-pulse flex gap-2">
                ...
                <div className="flex-1 space-y-1">
                  <div className="h-2.5 bg-zinc-200 dark:bg-zinc-700 rounded-full w-full" />
          ...
          <div className="space-y-3">
            {activity.map((item, i) => (
              <div key={item._id || i} className="flex gap-2.5">
                <div className="w-7 h-7 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-[9px] font-bold shrink-0 overflow-hidden">
                  {item.authorAvatar ? (
      ...
      {/* Popular Tags */}
      {allTags.length > 0 && (
        <Section title="Popular Tags" icon={<FiHash size={14} />}>
          <div className="flex flex-wrap gap-1.5">
            {allTags.map(({ tag, count }) => (
              <span
                key={tag}
                className="inline-flex items-center gap-1 px-2.5 py-1 bg-zinc-100 dark:bg-zinc-800/80 text-zinc-600 dark:text-zinc-400 rounded-lg text-xs font-medium hover:bg-indigo-50 dark:hover:bg-indigo-900/20 hover:text-indigo-600 dark:hover:text-indigo-400 cursor-pointer transition-colors"
              >
                #{tag}
      ...
      {/* Quick Links */}
      <Section title="Quick Links" icon={<FiCompass size={14} />}>
        <div className="space-y-0.5">
          {QUICK_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="flex items-center gap-2.5 px-3 py-2 rounded-xl text-sm font-medium text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors"
            >
      ...
      {/* About */}
      <Section title="About" icon={<FiInfo size={14} />}>
...
function Section({ title, icon, children }) {
  return (
    <div className="bg-white dark:bg-zinc-900/80 backdrop-blur-sm rounded-2xl border border-zinc-100 dark:border-zinc-800/60 overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-3 border-b border-zinc-100 dark:border-zinc-800/60">
