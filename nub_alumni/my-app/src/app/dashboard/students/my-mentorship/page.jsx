...
import {
  ...
  FiLoader, FiMessageSquare, FiUser, FiCalendar,
  FiChevronRight, FiStar, FiSearch, FiArrowRight, FiVideo,
} from "react-icons/fi";
...
function MentorshipCard({ mentorship, onCancel, acting }) {
    ...
    >
      <div className="p-5">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-zinc-900 dark:text-white">{mentorship.alumniName}</h3>
              <p className="text-[11px] text-zinc-500 dark:text-zinc-400">{mentorship.alumniEmail}</p>
            </div>
          </div>
      ...
      {mentorship.status === "pending" && (
        <div className="px-5 pb-4">
          <button
            onClick={() => onCancel(mentorship._id)}
            disabled={acting === mentorship._id}
            className="w-full flex items-center justify-center gap-1.5 px-4 py-2 bg-white dark:bg-zinc-800 hover:bg-red-50 dark:hover:bg-red-950/20 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800 rounded-xl text-xs font-semibold transition-all disabled:opacity-50"
          >
        ...
        <div className="px-5 pb-4 space-y-3">
          <div className="flex items-center gap-2 p-3 bg-emerald-50 dark:bg-emerald-950/20 rounded-xl border border-emerald-200 dark:border-emerald-800">
            ...
            <span className="text-xs font-semibold text-emerald-700 dark:text-emerald-400">
              Mentorship in progress - connect with your mentor!
            </span>
          </div>
          <div className="flex gap-2">
            <Link
              href={`/dashboard/students/text-box?chatWith=${mentorship.alumniEmail}`}
...
export default function MyMentorshipPage() {
  ...
  useEffect(() => {
    ...
    const fetchMentorships = async () => {
      try {
        const data = await apiFetch(`/api/mentorships?mode=student&email=${encodeURIComponent(user.email)}`);
  ...
  const handleCancel = async (id) => {
    ...
    try {
      ...
      setMentorships((prev) =>
        prev.map((m) =>
          m._id === id ? { ...m, status: "declined", updatedAt: new Date().toISOString() } : m
        )
      );
  ...
  return (
    <div className="relative min-h-screen space-y-6 lg:space-y-8 overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div className="space-y-2">
              <motion.div variants={fadeUp} custom={0} className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-full px-4 py-1.5 border border-white/20">
                ...
                <span className="text-xs font-semibold text-blue-100">My Mentorship</span>
              </motion.div>
              ...
              <motion.p variants={fadeUp} custom={2} className="text-blue-100/80 text-sm max-w-lg leading-relaxed">
                Track your mentorship requests and connect with alumni mentors.
              </motion.p>
            </div>
            <motion.div variants={fadeUp} custom={3}>
              <Link
                href="/alumni-directory"
                className="inline-flex items-center gap-2 bg-white text-blue-600 text-sm font-semibold px-5 py-3 rounded-2xl hover:bg-blue-50 transition-all duration-300 shadow-lg hover:shadow-xl hover:-translate-y-0.5"
              >
      ...
      {filtered.length === 0 ? (
        ...
        >
          <div className="w-20 h-20 rounded-full bg-blue-50 dark:bg-blue-950/30 flex items-center justify-center mx-auto mb-4">
            <FiBookOpen className="w-10 h-10 text-blue-300" />
          </div>
          <h3 className="text-lg font-semibold text-zinc-700 dark:text-zinc-300 mb-1">
            {activeTab === "all" ? "No mentorships yet" : `No ${activeTab} mentorships`}
          </h3>
          <p className="text-sm text-zinc-400 mb-4">
            {activeTab === "all"
              ? "Start your mentorship journey by connecting with alumni."
              : "Try a different tab to see other mentorships."}
          </p>
          {activeTab === "all" && (
            <Link
              href="/alumni-directory"
              className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold px-6 py-3 rounded-xl transition-colors shadow-md shadow-blue-600/20"
            >
              <FiSearch className="w-4 h-4" />
